...
```
C closer to 1 indicates that the SVM employed moderate regularization to balance generalization error & overfitting trade off 

The sigma value indicates how much the model places emphasis on individual data points. A lower value means that more emphasis was placed on individual data points instead of broader clusters 
...
```{r}
# Extract feature importance from rf_tuned_death using caret's varImp()
importance_death_rf <- varImp(rf_tuned_death)

# Display the importance scores
cat("\nRandom Forest (Death Rate) Feature Importance:\n")
print(importance_death_rf)

# Optionally, plot the feature importance
plot(importance_death_rf, main = "Feature Importance for Random Forest (Death Rate)")
...
```{r}
# Extract and display coefficients for Logistic Regression (Death Rate)
cat("\nLogistic Regression (Death Rate) Coefficients:\n")
print(summary(lr_model_death$finalModel))
...
```{r}
# Extract permutation importance using varImp() for SVM (Death Rate)
svm_importance_death <- varImp(svm_model_death, scale = TRUE)

# Display the importance scores
cat("\nSVM (Death Rate) Permutation Importance:\n")
print(svm_importance_death)

# Optionally, plot the feature importance
plot(svm_importance_death, main = "Permutation Importance for SVM (Radial Kernel, Death Rate)")
```
