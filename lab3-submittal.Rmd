...
model_data <- dataset %>%
  select(median_age, median_income, total_pop, Infection_Rate_Class, infection_rate)
...
model_data <- model_data %>%
  mutate(
    Infection_Rate_Class = factor(ifelse(infection_rate > 0.07212, "High", "Low"))
  )
...
# Save the final table (optional, for reference)
